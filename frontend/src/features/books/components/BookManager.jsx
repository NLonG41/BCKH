...
const BookManager = ({ books, onUpdated, onMessage }) => {
  const { t } = useI18n();
  const { createBook, updateBook, deleteBook } = useBooks();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        await updateBook(editingId, { ...form, quantity: Number(form.quantity) });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      onMessage?.(error.response?.data?.message || t("cannotDeleteBook"));
    }
  };
